...
function parseStatusesRedisInput(resultObject) {
    ...
    for(key in resultObject) {
        ...
        updateChannels(targetNetwork, channel, nick, function (ch) {
            if(!ch.users){
                ch.users = [];
            }
...
function prettyPrintKarmaScores(scores) {
    ...
    _.forEach(scores, function(score) {
        if (score.user.length + score.score.toString().length + space > maxLength)  {
            maxLength = score.user.length + score.score.toString().length + space;
        }
    });
