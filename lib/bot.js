...
function Bot(nick) {
    ...
    this.parseMentionOnFactoidMentionRequests = function (message) {
        var matchData = message.split("@");
        if (matchData) {
            return matchData[1].trim();
        }
    }
    this.parseFactoionOnFactoidMentionRequests = function (message) {
        var matchData = message.split("@");
        if (matchData) {
            return matchData[0].trim();
        }
    }
}
