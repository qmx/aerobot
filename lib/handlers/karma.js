...
function queryForKarma(network, channel, ordering, cb) {
    db.Karma.findAll({
        where: {
            network: network,
            channel: channel
...
module.exports = function(config, irc, bot, db, redis, from, to, message) {
    ...
    if(bot.isKarmaRequest(message)) {
        ...
        queryForKarma(config.irc.host, util.normalizeChannelName(to), 'DESC', function(err, scores) {
            irc.say(to, util.prettyPrintKarmaScores(scores));
        });
    } else if (bot.isKarmaWorstRequest(message)) {
        queryForKarma(config.irc.host, util.normalizeChannelName(to), 'ASC', function(err, scores) {
            irc.say(to, util.prettyPrintKarmaScores(scores));
        });
    }
};